...
def model(time_in, ydata=None):
    ...
    # Order: krad, beta_n_t1, e_n_t1, beta_n_t2, beta_p_t2, N_t2
    # Held at zero: bkg
    theta = numpyro.sample(
        "theta",
        TruncatedNormal(
            low   = jnp.array([-21.00, -5.00, -6.50, -18.50, -21.50, 14.00]),
            high  = jnp.array([-17.00, 0.00, -1.00, -16.00, -17.00, 17.00]),
            loc   = jnp.array([-20.00, -2.00, -4.00, -17.00, -19.00, 14.50]),
            scale = theta_stddev * jnp.ones(6),
        ),
    )

    krad, beta_n_t1, e_n_t1, beta_n_t2, beta_p_t2, N_t2 = 10**theta
    bkg = 0.0
    ...
    def get_signal_trace(n0_val):
        raw_output = runner(time_in, krad, beta_n_t1, e_n_t1, beta_n_t2, beta_p_t2, N_t2, n0_val, bkg)[0]
...
print("Starting MCMC (DTDeepVar Model)...")
...
# 3. Save to NetCDF
filename = f"{filename[:-4]}_DTDeepVar_chains{num_devices}_WU{warmups}_SAM{samples}.nc"
