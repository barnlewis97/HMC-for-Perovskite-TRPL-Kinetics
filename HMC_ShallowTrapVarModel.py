...
runner = jit(TRPL_ShallowTrapVar)
...
def model(time_in, ydata=None):
    ...
    # Order: k_c, k_e, k_rad, NT
    # Held at zero: k_aug, p0, bkg
    theta = numpyro.sample(
        "theta",
        TruncatedNormal(
            low   = jnp.array([-18.50, -6.50, -21.00, 14.00]),
            high  = jnp.array([-16.00, -1.00, -17.00, 17.00]),
            loc   = jnp.array([-17.00, -4.00, -20.00, 14.50]),
            scale = theta_stddev * jnp.ones(4),
        ),
    )

    k_c, k_e, k_rad, NT = 10**theta
    k_aug = p0 = bkg = 0.0
    ...
    def get_signal_trace(n0_val):
        raw_output = runner(time_in, n0_val, k_c, k_e, k_rad, k_aug, NT, p0, bkg)[0]
...
print("Starting MCMC (ShallowTrapVar Model)...")
...
# 3. Save to NetCDF
filename = f"{filename[:-4]}_ShallowTrapVar_chains{num_devices}_WU{warmups}_SAM{samples}.nc"
